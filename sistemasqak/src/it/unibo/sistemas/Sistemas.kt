				...
				state("handleEval") { //this:State
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("arg(V)"), Term.createTerm("arg(X)"), 
												...
												val V = arg.toDouble()
								CommUtils.outcyan("$name : evaluate: $V")
								 val Value = eval(V)  
								CommUtils.outgreen("$name : value estimated = $Value")
						}
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
				...
				state("handleRequest") { //this:State
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("argr(V)"), Term.createTerm("argr(X)"), 
								...
								CommUtils.outcyan("$name : evaluate: $V")
								 Res = eval(V)  
								CommUtils.outgreen("$name : value estimated = $Res")
						}
						answer("evalr", "evalreply", "value($Res)"   )  
