...
//        webSocket.sendText(c56, true);
    ...
    protected void sendCellChange( ) throws InterruptedException {
    	...
    	//tentativo di connessione con server tramite web socket
        WebSocket webSocket = client.newWebSocketBuilder()	//crea una connessione websocket che tenta di connettersi con un server che lavora sulla porta 8080
            .buildAsync(URI.create("ws://localhost:8080/eval"), new WebSocketListener(latch)) //apre anche un listener, che permette di porsi in ascolto sul server
            .join();
